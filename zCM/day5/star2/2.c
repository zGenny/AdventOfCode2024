...
void ordina_aggiornamento(int *aggiornamento, int size, Regola *regole, int contatore_regole) {
    int ordinato;
    do {
        ordinato = 1;
        for (int i = 0; i < size - 1; i++) {
            for (int j = 0; j < contatore_regole; j++) {
                if (aggiornamento[i] == regole[j].data && aggiornamento[i + 1] == regole[j].link) {
                    int temp = aggiornamento[i];
                    aggiornamento[i] = aggiornamento[i + 1];
                    aggiornamento[i + 1] = temp;
                    ordinato = 0; 
                    break;
                }
            }
...
int main() {
    ...
    for (int i = 0; i < conta_aggiornamento; i++) {
        if (controllo(aggiornamenti[i], aggiornamento_size[i], regole, contatore_regole)) {
            ...
            ordina_aggiornamento(aggiornamenti[i], aggiornamento_size[i], regole, contatore_regole);
            somma_ordinata += trova_numero(aggiornamenti[i], aggiornamento_size[i]);
        }
    ...
    printf("Somma dei numeri centrali degli aggiornamenti ordinati: %d\n", somma_ordinata);
    return 0;
}
